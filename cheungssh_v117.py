...
try:
    import paramiko, threading, socket, ConfigParser, time, commands, threading, re, getpass, Format_Char_Show, shutil, \
        random, getpass, LogCollect, readline, filemd5, command_tab
except Exception, e:
...
def SSH_cmd(ip, username, password, port, cmd, UseLocalScript, OPTime):
    ...
    try:
        ...
        if err:
            FailIP.append(ip)
            ResultSum_count = "\033[1m\033[1;32m+OK %s (%0.2f Sec, All %d Done %d \033[1m\033[1;31mCmd:Failed\033[1m\033[1;32m)\033[1m\033[0m" % (
            ...
            error_out = 'NULL'
            ResultSum_count = "\033[1m\033[1;32m+OK %s (%0.2f Sec, All %d Done %d  Cmd:Sucess)\033[1m\033[0m" % (
            ip, float(time.time() - start_time), All_Servers_num_all, All_Servers_num)
        ...
        FailIP.append(ip)
        All_Servers_num += 1
        ResultSum_count = "\n\033[1m\033[1;31m-ERR %s %s (%0.2f Sec All %d Done %d)\033[1m\033[0m\a" % (
...
def Read_config(file="/cheung/conf/cheung.conf"):
    global Servers, Useroot, Timeout, RunMode, UseKey, Deployment, ListenTime, \
        ...
        ServersRootPassword, NoPassword, NoRootPassword, HostsGroup, HOSTSMD5, CONFMD5
    ServersPort = {}
    ServersPassword = {}
    ServersUsername = {}
    ServersRootPassword = {}
...
def Main_p():
    global s_file, d_file, All_Servers_num_Succ, LocalScript, Global_start_time, NoPassword, NoRootPassword, \
        All_Servers_num_all, All_Servers_num, Servers, HostsGroup
    ...
    try:
        ...
        if NoPassword and UseKey == "N":
            SetPassword = getpass.getpass(
                "请在此处为所有主机指定密码(请确保该密码适用用于所有的服务器，否则请在配置文件/cheung/conf/hosts文件中逐个指定)\n\033[1;33mHosts Password:\033[0m  ")
            if SetPassword:
                print "已为所有主机指定密码"
            else:
                print "您尚未指定密码，程序退出"
...
def Excute_cmd_root(s, Port, Username, Password, Passwordroot, cmd, UseLocalScript, OPTime):
    ...
    try:
        ...
        FailIP.append(s)
        ResultSum = "\n\033[1m\033[1;31m-ERR %s %s (%0.2f Sec All %d Done %d)\033[1m\033[0m\a" % (
        e, s, float(time.time() - start_time), All_Servers_num_all, All_Servers_num)
        bufflog = str(e)
    if Result_status:
