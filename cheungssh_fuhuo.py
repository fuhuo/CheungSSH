...
os.sys.path.insert(0, os.path.abspath(localpath))
os.sys.path.insert(0, os.path.abspath(localpath + '/libs'))
os.sys.path.insert(0, os.path.abspath(localpath + '/cheung/bin/'))
try:
    import paramiko
    import readline
except Exception, e:
...
FlagPath = '%s/flag' % localpath
LogFile = '%s/logs/cheungssh.log' % localpath
SLogFile = '%s/logs/cheungssh.source.log' % localpath
DeploymentFlag = "%s/tmp/DeploymentFlag%s" % (localpath,str(random.randint(999999999, 999999999999)))
HostsFile = "%s/conf/hosts" % localpath
ConfFile = "%s/conf/cheung.conf" % localpath
VersionFile = "%s/version/version" % localpath
ScriptPath = "%s/scripts" % localpath
try:
    paramiko.util.log_to_file('%s/logs/paramiko.log' % localpath)
except Exception, e:
    pass
...
def Write_Log(ip, stderr, stdout, Logcmd, LogFile, useroot, username,
              UseLocalScript, Deployment, DeploymentStatus, OPTime):
    os.system("chmod 777 %s/logs/* 2>/dev/null"%localpath)
    if DeploymentStatus:
    ...
    try:
        ...
        T.write(
            ...
            "时间:" + OPTime + '===' + \
            "是否使用su-root:" + useroot + '===' + \
            "是否使用脚本:" + UseLocalScript + '===' + \
            "是否使用部署模式:" + Deployment + '===' + \
            "部署完成状态" + DeploymentStatus_T + '===' + \
            "命令:" + Logcmd + '===' + \
            "错误显示:" + stderr + '===' + \
            "正确显示:" + stdout)
        T.close()
...
def InitInstall():
    global ConfFile
    INITDIR = "%s/logs %s/flag  %s/conf %s/bin %s/version"%(localpath,localpath,localpath,localpath,localpath)
...
#ListenTime=60
#ListenChar=Server startup"""%HostsFile)
        T.close()
    try:
        VerR = int(open(VersionFile).read().strip())
    except Exception, e:
        VerR = 0
    if VerR < 104:
        os.system("""echo %s >%s 2>/dev/null""" % (VERSION,VersionFile))
        T = open(ConfFile, 'w')
        T.write("""[CheungSSH]
#Author=Cheung Kei-Chuen
#QQ=2418731289
...
def SSH_cmd(ip, username, password, port, cmd, UseLocalScript, OPTime):
    ...
    try:
        ...
        if err:
            FailIP.append(ip)
            ResultSum_count = "\033[1m\033[1;32m+OK [%s|%s] (%0.2f Sec, %d/%d \033[1m\033[1;31mCmd:Failed\033[1m\033[1;32m)\033[1m\033[0m" % (
            ip, username, float(time.time() - start_time), All_Servers_num, All_Servers_num_all)
            out = 'Null\n'
            if Deployment == 'Y':
                DeploymentStatus = False
            Write_Log(ip, ResultSumLog.strip('\\n'), out, cmd, LogFile, 'N', username, UseLocalScript, Deployment,
                      DeploymentStatus, OPTime)
        else:
            error_out = 'NULL'
            ResultSum_count = "\033[1m\033[1;32m+OK [%s|%s] (%0.2f Sec, %d/%d  Cmd:Sucess)\033[1m\033[0m" % (
            ip, username, float(time.time() - start_time), All_Servers_num, All_Servers_num_all)
            All_Servers_num_Succ += 1
            if Deployment == 'Y':
                print  "Wating %s deployment (for %d Sec)..." % (ip, ListenTime)
                T = LogCollect.LogCollect(ip, port, username, password,
                                          """grep  -E "%s"  %s -q && echo  -n 'DoneSucc'""" % (
                                          ListenChar, DeploymentFlag), ListenTime, UseKey)
                if T:
                    DeploymentStatus = True
                else:
                    DeploymentInfo = "Main commands excuted success, But deployment havn't check suncess info (%s) " % (
                    ListenChar)
                    DeploymentStatus = False

            Write_Log(ip, error_out, ResultSumLog.strip('\\n') + '\n', cmd, LogFile, 'N', username, UseLocalScript,
                      Deployment, DeploymentStatus, OPTime)
        ...
        FailIP.append(ip)
        All_Servers_num += 1
        ResultSum_count = "\n\033[1m\033[1;31m-ERR [%s|%s] (%0.2f Sec %d/%d)\033[1m\033[0m\a" % (
        ip, e, float(time.time() - start_time), All_Servers_num, All_Servers_num_all)
    ...
    if Deployment == 'Y' and not DeploymentStatus:
        while True:
            TT = raw_input(
                "%s Deployment not Success (%s) want contiue deployment next server (yes/no) ? " % (ip, DeploymentInfo))
            if TT == 'yes':
                break
            elif TT == 'no':
                sys.exit(1)
    if All_Servers_num == All_Servers_num_all:  # 这里防止计数器永远相加下去
        ...
        print "+Done (Succ:%d,%s, %0.2fSec CheungSSH(V:%d) Cheung Kei-Chuen All Right Reserved)" % (
        All_Servers_num_Succ, FailNumShow, time.time() - Global_start_time, VERSION)
        All_Servers_num = 0
...
def Read_config():
    global Servers, Useroot, Timeout, RunMode, UseKey, Deployment, ListenTime, \
        ListenFile, ListenChar, ServersPort, ServersPassword, ServersUsername, \
        ServersRootPassword, NoPassword, NoRootPassword, HostsGroup, HOSTSMD5, CONFMD5, sudo
    file = ConfFile
    ServersPort = {}
    ServersPassword = {}
    ServersUsername = {}
    ...
    if Useroot=="Y" and sudo=="Y":
        print "您已经su - root了，不能配置sudo，可以sudo=N后，使用su - root"
        sys.exit(1)
    else:
        sudo = None
    try:
...
def Upload_file(ip, port, username, password):
    ...
    try:
        ...
        All_Servers_num += 1
        All_Servers_num_Succ += 1
        print SftpInfo + "\033[1m\033[1;32m+OK %s (%0.2f Sec  All %d Done %d)\033[1m\033[0m" % (
        ip, time.time() - start_time, All_Servers_num_all, All_Servers_num)
    except Exception, e:
        All_Servers_num += 1
        print "\033[1m\033[1;31m-ERR %s %s(%0.2f Sec,All %d Done %d)\033[1m\033[0m" % (
        ip, e, float(time.time() - start_time), All_Servers_num_all, All_Servers_num)
    else:
        t.close()
...
def Download_file_regex(ip, port, username, password):
    ...
    try:
        ...
        All_Servers_num += 1
        All_Servers_num_Succ += 1
        print "\033[1m\033[1;32m+OK %s (%0.2f Sec All %d Done %d)\033[1m\033[0m" % (
        ip, float(time.time()) - start_time, All_Servers_num_all, All_Servers_num)
    except Exception, e:
        All_Servers_num += 1
        print "\033[1m\033[1;31m-ERR %s %s (%0.2f Sec All %d Done %d)\033[1m\033[0m" % (
        ip, e, float(time.time() - start_time), All_Servers_num_all, All_Servers_num)
...
def Main_p():
    global s_file, d_file, All_Servers_num_Succ, LocalScript, Global_start_time, NoPassword, NoRootPassword, \
        All_Servers_num_all, All_Servers_num, Servers, HostsGroup, ServersPassword, ServersRootPassword
    # All_Servers_num_all=len(Servers.split(','))
    All_Servers_num = 0
    All_Servers_num_Succ = 0
    if not Servers:
        print "当前没有配置服务器地址,请在%s文件中配置!" % HostsFile
        sys.exit()
    os.system("rm -f %s/data/hosts/* 2&>/dev/null" % localpath)
    for host in Servers:
        try:
            os.mknod("%s/data/hosts/%s" % (localpath, host))
        except:
            pass
    os.system("touch %s/data/hosts/fail %s/data/hosts/all 2&>/dev/null" % (localpath,localpath))
    for group in HostsGroup:
        try:
            os.mknod("%s/data/hosts/%s" % (localpath, group))
        except:
            pass
    try:
        ...
        p.add_option("-g", "--group", help="""Description:  set the target group
            Example:
                %s -g "group-name"
                %s -t cmd -g "group-name" -c "df -h"
                %s -t upload -g "group-name" -s /local/file  -d /remote/dir
              """%(sys.argv[0], sys.argv[0], sys.argv[0]))
        p.add_option("-c", "--command", help="""Description: run the command
            Example:
              %s -g "group-name" -c "df -h"
        """%sys.argv[0])
        p.add_option("-o", "--hosts", help="""Description: set the target host or ip
            Example:
                %s -o 127.0.0.1
                %s -o 127.0.0.1,127.0.0.2
                %s -g "group-name" -o 127.0.0.1
                %s -t cmd -o 127.0.0.1 -c "df -h"
        """%(sys.argv[0], sys.argv[0], sys.argv[0], sys.argv[0]))

        (option, args) = p.parse_args()

        if option.group:
            if option.group in HostsGroup.keys():
                Servers = HostsGroup[option.group]
                print "指定执行主机组为%s：所有主机列表：%s"%(option.group,Servers)
            else:
                print "%s不是hosts配置文件里的指定组，请重新确认"%option.group
                sys.exit()

        if option.hosts:
            HostList = option.hosts.split(',')
            Temp_Servers = []
            Not_In_Servers = []
            for host in HostList:
                host = host.strip()
                if host not in Servers:
                    Not_In_Servers.append(host)
                else:
                    Temp_Servers.append(host)
            Servers = Temp_Servers
            if Servers:
                if Not_In_Servers:
                    print "hosts文件中没有在组%s中查找到的IP %s"%("all" if not option.group else option.group, Not_In_Servers)
                print "所有指定主机列表：%s"%(Servers)
            else:
        ...
        if NoPassword and UseKey == "N":
            SetPassword = getpass.getpass(
                "请在此处为在密码列填写了[None]的主机指定密码;\n如果没有填写None的主机,密码依然读取配置文件中的信息;\n(请确保您输入的密码适用于所有密码列填写了None的主机，否则请在配置文件%s文件中逐个指定)\n\033[1;33mHosts Password:\033[0m  " % HostsFile)
            if SetPassword:
                print "已为所有主机指定密码"
            else:
                print "您尚未指定密码，程序退出"
                ...
                if SetRootPassword:
                    ...
                    for host in Servers:
                        if ServersRootPassword[a] is None:
                            ServersPassword[host] = SetRootPassword
                else:
        ...
        if option.excute_type == "cmd":
                ...
                if RunMode.upper() == 'M':
                    if UseKey == "Y":
                        if float(sys.version[:3]) < 2.6:
                            ...
                            a = threading.Thread(target=Upload_file,
                                                 args=(s, ServersPort[s], ServersUsername[s], ServersPassword[s]))
                            a.start()
            ...
            for s in Servers:
                if option.regex:
                    if UseKey == "Y":
                        ...
                        a = threading.Thread(target=Download_file,
                                             args=(s, ServersPort[s], ServersUsername[s], ServersPassword[s]))
                else:
                    if UseKey == "Y":
                        a = threading.Thread(target=Download_file_regex,
                                             args=(s, ServersPort[s], ServersUsername[s], None))
                    else:
                        a = threading.Thread(target=Download_file_regex,
                                             args=(s, ServersPort[s], ServersUsername[s], ServersPassword[s]))
            ...
            if not os.path.isfile("%s/.NoTabAsk" % FlagPath):
                TabAsk = raw_input(
                    "在新版本中，已经支持TAB补全功能，但是补全的依据是根据您当前所在的这个服务器上的路径为标准的，而并不是远程服务器上的路径，所以您需要确保使用TAB补全后，路径是正确的\n是否取消提示 (yes/no) ")
                if re.search("^ *[Yy]([Ee][Ss])? *$", TabAsk):
                    try:
                        os.mknod("%s/.NoTabAsk" % FlagPath)
                        print "已取消提醒"
                    except Exception, e:
                        print "取消提醒失败(%s)" % e
            if option.command:
...
def Excute_sudo(s,Port,username,Password,cmd,UseLocalScript,OPTime):
    ...
    if username=='root':
        print "不能用root登录后再sudo登录"
        sys.exit()
    elif sudoinfo[-1] in ('-', 'root', 'su'):
        prompt = '# '
    else:
        prompt = '$ '
    try:
        ...
        if Result_status:
            ...
            All_Servers_num += 1
            buffer = '\n'.join(buff.split('\r\n')[1:][:-1])
            ResultSum = buff + "\n\033[1m\033[1;32m+OK [%s|%s] (%0.2f Sec %d/%d)\033[1m\033[0m\n" % (s,username,float(time.time() - start_time),All_Servers_num,All_Servers_num_all)

            bufflog_new = ''
            for logline in bufflog.split():
                if logline==cmd:
                    continue
                bufflog_new += logline
            bufflog = bufflog_new
            ...
            FailIP.append(s)
            buff = ''
            ResultSum = buff + "\n\033[1m\033[1;31m-ERR sudo Failed (%s) [%s|%s] (%0.2f Sec %d/%d)\033[1m\033[0m\n" % (info,s,username,float(time.time() - start_time),All_Servers_num,All_Servers_num_all)
    except Exception,e:
        All_Servers_num += 1
        Result_status = False
        FailIP.append(s)
        ResultSum = "\n\033[1m\033[1;31m-ERR %s [%s|%s] (%0.2f Sec %d/%d)\033[1m\033[0m\a" % (e,s,username,float(time.time() - start_time),All_Servers_num,All_Servers_num_all)
        bufflog = str(e)
    if Result_status:
        Write_Log(s, 'NULL', bufflog.strip('\\n') + '\n', cmd, LogFile, 'Y', username, UserLocalScript, 'N', 'N', OPTime)
...
def Excute_cmd_root(s, Port, username, Password, Passwordroot, cmd, UseLocalScript, OPTime):
    ...
    try:
        ...
        Result_status = False
        FailIP.append(s)
        ResultSum = "\n\033[1m\033[1;31m-ERR %s [%s|%s] (%0.2f Sec All %d/%d)\033[1m\033[0m\a" % (
        e, s, username, float(time.time() - start_time), All_Servers_num, All_Servers_num_all)
        bufflog = str(e)
    if Result_status:
...
#        All_Servers_num_all = len(Servers_T)
        ...
        cmd = re.sub("^ *more", "cat", cmd)
        cmd = re.sub("^ *vmstat *[1-9]", "vmstat 1 10", cmd)
        try:
            if not IS_PWD:
                if re.search("^ *cd.*", cmd):
                    try:
                    ...
                    IS_PWD = True
                    if not os.path.isfile("%s/.NoAsk"%FlagPath):
                        AskNotice = raw_input(
                            "\033[1;33m注意: 请您确保切换的路径[%s]在远程服务器上是存在的，否则切换路径没有任何意义,您清楚了吗？\033[0m(yes/no) " % (
                            re.sub("^ *cd *|;", "", PWD)))
                        if re.search("[Yy]([Ee][Ss])?", AskNotice):
                            AskCancel = raw_input("是否取消以上提示？(yes/no) ")
                            if re.search("[Yy]([Ee][Ss])?", AskCancel):
                                try:
                                    os.mknod("%s/.NoAsk"%FlagPath)
                                    os.system("chmod 777 %s/.NoAsk"%FlagPath)
                                    print "已取消提醒"
                                except Exception, e:
                                    print "抱歉，不能取消提示(%s)" % e
                        else:
                            print "如果您对以上提示不清楚,那么那您可以在远程服务器上手动%s 那一定会报错的，所以请确保[%s]有效!" % (
                            re.sub(";", "", PWD), re.sub("^ *cd *|;", "", PWD))
                            sys.exit()
        ...
        if re.search('^ *[Ff][Ll][Uu][Ss][Hh] *[Ll][Oo][Gg][Ss] *$', cmd):
            try:
                ...
                shutil.move(LogFile, '%s.%s'%(LogFile,Log_Flag))
                print "+OK"
                continue
            except Exception, e:
        ...
        #    print "抱歉，当前不支持交互式"
        #    continue
        if re.search("^ *[Uu][Ss][Ee] +[Ss][Yy][Ss]", cmd):
            if UseSystem == True:
	...
	show                显示主机分布情况"""
                ...
                print "抱歉，CheungSSH暂时不支持您输入的内部命令,如果要执行*nux命令，请使用no use退出"
                continue
        else:
            IsBack = False
            if re.search("^ *[Ss][Hh][Oo][Ww] *", cmd):
...
def Run_cmd(cmd):
    ...
    if re.search("^ *[Rr][Uu][Nn] +", cmd):
        try:
            ...
            IsScript = False
            for host in ScriptFileCmd:
                if os.path.isfile(host):
                    ScriptFilePath = host
                    IsScript = True
                    bread
            if not IsScript:
                print "您指的定程序[%s]在本地不存在！" % ScriptFileCmd
                return
            else:
    ...
    if re.search("^ *vim? +",cmd):
        ...
        if not os.path.isfile("%s/.NoAskEdit"%FlagPath):
